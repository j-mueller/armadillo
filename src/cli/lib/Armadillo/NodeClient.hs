...
runArmadilloNodeClient :: K.LogEnv -> NodeClientConfig -> NodeClientStateFile -> [ChainPoint] -> IO (TVar ChainFollowerState)
runArmadilloNodeClient logEnv NodeClientConfig{nccCardanoNodeConfigFile, nccCardanoNodeSocket} nodeClientStateFile@(NodeClientStateFile cfeStateFile) chainPoints = do
  ...
  _ <- liftIO $ forkIO $ do
        runExceptT (runNodeClient nccCardanoNodeConfigFile nccCardanoNodeSocket (mkArmadilloNodeClient tv logEnv nodeClientStateFile chainPoints))
        >>= either (fail . show) pure
  pure tv

mkArmadilloNodeClient :: TVar ChainFollowerState -> K.LogEnv -> NodeClientStateFile -> [ChainPoint] -> LocalNodeConnectInfo CardanoMode -> Env -> IO PipelinedLedgerStateClient
mkArmadilloNodeClient cfeState logEnv (NodeClientStateFile cfeStateFile) chainPoints _connectInfo nodeEnv = K.runKatipContextT logEnv () "cli" $ runMonadLogKatipT $ do
  cfeScripts <- either (fail . show) pure Scripts.compileScripts

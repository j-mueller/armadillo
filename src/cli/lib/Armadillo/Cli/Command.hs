...
import           Options.Applicative    (CommandFields, Mod, Parser, auto,
                                         ...
                                         option, progDesc, strOption, subparser,
                                         value)
...
data RefScriptCommand =
  Deploy WalletClientOptions OperatorConfigSigning
  | Check
...
refScriptDeploy = command "deploy" $
  info (Deploy <$> parseWalletClientOptions <*> parseOperatorConfigSigning) (fullDesc <> progDesc "Deploy the reference scripts")

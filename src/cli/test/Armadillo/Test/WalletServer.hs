...
-}
module Armadillo.Test.WalletServer(
  RunningWalletServer(..),
  WalletLog(..),
  withWallet,
  getUTxOs,
...
withWallet tracer stateDirectory rn@RunningNode{rnNodeSocket, rnNodeConfigFile, rnNetworkId} action = do
  ...
  let rwsClient = mkClientEnv rwsManager (BaseUrl Http "localhost" rwsPort "")
  ...
  withLogFile logFilePath $ \out -> do
    ...
    let p = mkProc $ RunWallet walletCfg verificationConf rwsPort
...
walletClientOptions :: RunningWalletServer -> WalletClientOptions
walletClientOptions RunningWalletServer{rwsPort} =
  WalletClientOptions
    { wcoHost = "localhost"
    , wcoPort = rwsPort
    }
